    ...
    class CheckGitCommitAction < Action
      def self.run(params)
        ...
        unless expect_regex.match?(git_last_commit_log)
          UI.message("Unexpected git last commit log. Should be `#{expect_regex}` but found `#{git_last_commit_log}`.")
          return false
        end
        ...
        if git_dirty_files.length > 0
          UI.message("Found unexpected uncommitted changes in the working directory. git_dirty_files=#{git_dirty_files}")
          return false
        end
